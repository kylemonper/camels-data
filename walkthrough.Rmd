...
site_names <- c("USEPA-440432070255401", "test", "USGS-01011100", "USGS-010158001")
...
for (i in 1:length(site_names)) {
 ...
 if (inherits(data, "error")) {
   sites[[i]] <- NULL
     sprintf("site '%s' not found", site_names[i])
     next
 } else if(is.null(data)) {
     sprintf("no data found for site %s", site_names[i])
     next
   } else {
   sites[[i]] <- data %>% 
    filter(USGSPCode %in% param_codes$`5_digit_code`)
